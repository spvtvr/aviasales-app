...
export const getId = createAsyncThunk('tickets/getId', async (_, { dispatch }) => {
  await axios
    .get('https://aviasales-test-api.kata.academy/search')
    .then((res) => dispatch(setSearchId(res.data.searchId)));
});

export const getTickets = createAsyncThunk('tickets/getTickets', async function getTicketsAsync(id, { dispatch }) {
  await axios
...
export const ticketSlice = createSlice({
  ...
  reducers: {
    ...
    toggleFilter: (state, action) => {
      const toggledElem = state.filters.find((elem) => elem.id === action.payload.id);
    ...
    toggleButton: (state, action) => {
      const toggledButton = state.buttons.find((elem) => elem.id === action.payload);
...
export const { setSearchId, setTickets, setStop, setCountOnPage, toggleFilter, toggleButton } = ticketSlice.actions;
